...
let compare_result_to_json
  ...
  let opam_diff_to_json opam_diff =
    `List (List.map (fun diff -> `String diff) opam_diff)
  in
  let duniverse_to_json = function
    | Ok (left, right, detailed_diff) ->
      `Assoc [
        ...
        ("detailed_diff", `List (List.map (fun diff -> `String diff) detailed_diff))
      ]
    | Error (`Msg msg) ->
      `String msg
  in
  `Assoc [
    ...
    ("duniverse_diff", duniverse_to_json duniverse_ret)
  ]
